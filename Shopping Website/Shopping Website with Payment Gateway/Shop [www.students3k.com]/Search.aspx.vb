...
Partial Class Search
    ...
    Public comm As New SqlCommand(Sql, conn)
    Public dr As SqlDataReader
    Public Sub data(ByVal num As Integer, ByVal cat As String)
        constr = "server=.\SQLEXPRESS;database=shoping;trusted_connection=yes;"
        sql = "select top 8 * from item where code Not in(select top(" & num & ") code from item where  category='" & cat & "' and name='" & TextBox1.Text & "' order by code) and category='" & cat & "' and name='" & TextBox1.Text & "' order by code"
    ...
    Protected Sub lnk1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnk1.Click
        Session("line") = 0
        Session("cat") = cmbcat.SelectedValue
    ...
    Protected Sub ImageButton2_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ImageButton2.Click
        ...
        Else
            conn.Close()
            conn.Open()
            sql = "select name from cart where name='" & lbl1.Text & "' and uname='" & Session("userid") & "'"
            comm = New SqlCommand(sql, conn)
            dr = comm.ExecuteReader
            If dr.Read = True Then
                ImageButton2.Enabled = False
                ImageButton2.ToolTip = "This item is already added to the cart"
                dr.Close()
            Else
                dr.Close()
                'conn.Open()
                sql = "insert into cart values('" & lbl1.Text & "'," & lbl3.Text & ",'" & Session("userid") & "', " & 1 & ")"
                comm = New SqlCommand(sql, conn)
    ...
    Protected Sub ImageButton6_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ImageButton6.Click
        ...
        Else
            conn.Close()
            conn.Open()
            sql = "select name from cart where name='" & lbl7.Text & "' and uname='" & Session("userid") & "'"
            comm = New SqlCommand(sql, conn)
            dr = comm.ExecuteReader
            If dr.Read = True Then
                ImageButton6.Enabled = False
                ImageButton6.ToolTip = "This item is already added to the cart"
                dr.Close()
            Else
                dr.Close()
                ImageButton6.Enabled = False
                ...
                sql = "insert into cart values('" & lbl7.Text & "'," & lbl9.Text & ",'" & Session("userid") & "', " & 1 & ")"
                comm = New SqlCommand(sql, conn)
                comm.ExecuteNonQuery()
                conn.Close()
                Response.Redirect("search.aspx")
    ...
    Protected Sub ImageButton10_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ImageButton10.Click
        ...
        Else
            ...
            conn.Open()
            sql = "select name from cart where name='" & lbl13.Text & "' and uname='" & Session("userid") & "'"
            comm = New SqlCommand(sql, conn)
            dr = comm.ExecuteReader
            If dr.Read = True Then
                ImageButton10.Enabled = False
                ImageButton10.ToolTip = "This item is already added to the cart"
                dr.Close()
            Else
                dr.Close()
    ...
    Protected Sub ImageButton3_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ImageButton3.Click
        ...
        Else
            conn.Close()
            conn.Open()
            sql = "select name from cart where name='" & lbl4.Text & "' and uname='" & Session("userid") & "'"
            comm = New SqlCommand(sql, conn)
            dr = comm.ExecuteReader
            If dr.Read = True Then
                ImageButton3.Enabled = False
                ImageButton3.ToolTip = "This item is already added to the cart"
                dr.Close()
            Else
                dr.Close()
                Dim conn As New SqlConnection
    ...
    Protected Sub ImageButton8_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ImageButton8.Click
        ...
        Else
            conn.Close()
            conn.Open()
            sql = "select name from cart where name='" & lbl10.Text & "' and uname='" & Session("userid") & "'"
            comm = New SqlCommand(sql, conn)
            dr = comm.ExecuteReader
            If dr.Read = True Then
                ImageButton8.Enabled = False
                ImageButton8.ToolTip = "This item is already added to the cart"
                dr.Close()
            Else
                dr.Close()
                Dim conn As New SqlConnection
    ...
    Protected Sub ImageButton12_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ImageButton12.Click
        ...
        Else
            conn.Close()
            conn.Open()
            sql = "select name from cart where name='" & lbl16.Text & "' and uname='" & Session("userid") & "'"
            comm = New SqlCommand(sql, conn)
            dr = comm.ExecuteReader
            If dr.Read = True Then
                ImageButton12.Enabled = False
                ImageButton12.ToolTip = "This item is already added to the cart"
                dr.Close()
            Else
                dr.Close()
                Dim conn As New SqlConnection
    ...
    Protected Sub ImageButton14_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ImageButton14.Click
        ...
        Else
            conn.Close()
            conn.Open()
            sql = "select name from cart where name='" & lbl19.Text & "' and uname='" & Session("userid") & "'"
            comm = New SqlCommand(sql, conn)
            dr = comm.ExecuteReader
            If dr.Read = True Then
                ImageButton14.Enabled = False
                ImageButton14.ToolTip = "This item is already added to the cart"
                dr.Close()
            Else
                dr.Close()
                Dim conn As New SqlConnection
    ...
    Protected Sub ImageButton16_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ImageButton16.Click
        ...
        Else
            conn.Close()
            conn.Open()
            sql = "select name from cart where name='" & lbl22.Text & "' and uname='" & Session("userid") & "'"
            comm = New SqlCommand(sql, conn)
            dr = comm.ExecuteReader
            If dr.Read = True Then
                ImageButton16.Enabled = False
                ImageButton16.ToolTip = "This item is already added to the cart"
                dr.Close()
            Else
                dr.Close()
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        conn.ConnectionString = constr
        conn.Open()
        Dim sql = "select count(*) from item where category='" & cmbcat.SelectedValue & "'"
        Dim comm As New SqlCommand(sql, conn)
        Dim dr As SqlDataReader
        dr = comm.ExecuteReader
        If dr.Read() Then
